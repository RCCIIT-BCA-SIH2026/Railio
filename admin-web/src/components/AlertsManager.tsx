    ...
    <div className="space-y-6">
          ...
          return (
            ...
            >
              <div className="flex items-start space-x-4">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shrink-0 ${
                    isHigh ? 'bg-red-600 shadow-md shadow-red-600/30' : 'bg-amber-500 shadow-md shadow-amber-500/30'
                  }`}
                >
                ...
                <div>
                  <div className="flex items-center space-x-2 mb-1">
                    <span className="font-heading font-bold text-rail-text text-base">{alt.title}</span>
                    <span
                    ...
                    <span className="text-[10px] text-rail-muted font-mono">({alt.category})</span>
                  </div>
                  <p className="text-xs text-rail-muted mb-2">{alt.description}</p>
                  {alt.recommendedAction && (
                    <div className="text-xs text-amber-700 font-medium bg-amber-100 p-2 rounded-lg border border-amber-200">
              ...
              <div className="text-right shrink-0">
                <span className="text-xs text-rail-muted font-mono">{alt.timestamp}</span>
              </div>
            </div>
          );
        })}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="glass-panel bg-white w-full max-w-lg rounded-2xl p-6 border border-slate-200 shadow-2xl space-y-4">
            <h3 className="font-heading font-bold text-rail-text text-lg">Broadcast Incident Advisory</h3>
            <form onSubmit={handleCreate} className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-rail-muted">Alert Title</label>
                <input
                  type="text"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="e.g. Heavy Rain Speed Restriction at Howrah"
                  className="w-full mt-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-rail-text focus:outline-none focus:border-rail-orange"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-rail-muted">Category</label>
                  <select
                    value={category}
                ...
                <div>
                  <label className="text-xs font-semibold text-rail-muted">Severity</label>
                  <select
                    value={severity}
              ...
              <div>
                <label className="text-xs font-semibold text-rail-muted">Description & Guidance</label>
                <textarea
                  value={description}
              ...
              <div className="flex space-x-3 pt-3">
                <button
                  ...
                  className="flex-1 py-2 bg-slate-100 hover:bg-slate-200 text-rail-text rounded-lg text-xs font-semibold border border-slate-200"
                >
                  Cancel
                </button>
                <button
